...
async function checkUsernameAvailability() {
  ...
  if (!validUsernamePattern.test(username)) {
    usernameCheck.textContent =
      "ユーザーネームは日本語（ひらがな、カタカナ、漢字）・英数字のみ使用できます。";
  ...
  try {
    ...
    if (
      usernameDoc.exists() &&
      usernameDoc.data().rawUserId.includes(username)
    ) {
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    if (
      !validateInput(userIdInput, "ユーザーID") ||
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (userData.password === hashedPassword) {
          ...
          if (friend_query) {
            window.location.href = `../talk/index.html?friendid=${friend_query}`;
          } else {
            window.location.href = "../talk/index.html";
          }
        } else {
